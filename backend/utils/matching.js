...
// Advanced matching algorithm (Crash-proof)
export const calculateMatchScore = (user1, user2) => {
  ...
  if (coord1 && coord2 && Array.isArray(coord1) && Array.isArray(coord2)) {
    try {
      ...
      // Ignore distance calculation errors safely
    }
  }
  ...
  // 3. Training volume compatibility (15 points)
  ...
  if (user1.availability && user2.availability && user1.availability.length > 0 && user2.availability.length > 0) {
    ...
    score += (overlap.length / 7) * 20; // Max 7 days
  }

  // 5. Experience level compatibility (10 points)
...
export const findMatches = async (user, limit = 20) => {
  try {
    ...
    // Safely add gender filter
    if (user.matchPreferences?.gender) {
      query.gender = user.matchPreferences.gender;
    } else {
      query.gender = { $in: ['male', 'female', 'other'] };
    }
    ...
    const scored = users
      ...
      .filter(m => m.score > 10) // Lowered to 10% so you see results even with sparse data
